...
En Python, la fonction `reduce` a une histoire particulière. **Guido van Rossum, créateur de Python, déconseille fortement l’utilisation de `reduce`** — elle a été retirée des fonctions intégrées de Python 3 (et déplacée vers le module `functools`). On préfère les boucles explicites ou aux compréhensions pour tout ce qui n’est pas trivial, car elles sont plus lisibles et souvent plus rapides.
