...
En effet, l'IA ne me remplacera peut-être pas, mais la façon dont nous construisons les solutions de demain le fera et cela impliquera de plus en plus l'IA, en particulier par le biais d'agents, autonomes ou non.
...
Au fur et à mesure que je progressais dans la construction de l'application, je m'assurais que Claude suivait le workflow de développement et suggérait un nouvel ADR au moment opportun. J'ai également ajusté les prompts système et de l'espace de travail lorsque je constatais une lacune qui devait être enregistrée dans sa mémoire.

[Cette validation](https://github.com/JeremieLitzler/SocialMediaPublisherApp/tree/b4d7a11f5b6d4f38e3d3bafd112b905c29d74698/docs) vous montre à quoi ressemblait le dossier `docs` lorsque je me trouvai prêt à lancer la phase de codage. À ce stade, il me suffisait de configurer Claude et de le lancer.
...
Sur la base de [cet article](https://dev.to/suzuki0430/prevent-unexpected-claude-code-costs-with-this-vscodecursor-extension-nlb) d'[Atsushi Suzuki](https://dev.to/suzuki0430), j'ai téléchargé et installé le code pour ajouter une extension Visual Studio Code permettant de calculer le coût d'utilisation de Claude.

La configuration expliquée dans l'article est suffisante. Cependant, comme nous le verrons plus loin, la précision des coûts n'est pas satisfaisante.
...
Après un nouveau redémarrage, j'étais enfin prêt...
...
Mais l'extension censée calculer le coût me donnait un coût assez impréci...
...
Le résultat s'avère inférieur de plus de 30 % que la réalité, mais il est nettement plus proche des statistiques de Claude Console que de celles de l'extension.

Après les tâches 2 à 8, j'ai atteint 13,80 $, tandis que l'interface CLI `ccusage` m'indiquait que j'avais dépensé 10,44 $, soit une différence de 32 %. J'envigage probablement signaler cela à l'équipe `ccusage`.

Mais après avoir lu quelques articles sur leur dép^t GitHub, je me rends compte qu'il semble très difficile de calculer un chiffre précis et qu'Anthropic peut modifier les règles sans que personne ne sache vraiment quand et comment exactement.
...
Pour cela, je devais indiquer le flux de travail à suivre dans le fichier de prompts système. J'y ai donc ajouté que Claude Code devait suivre l'approche Git Flow et créer lui-même les tâches.

Au fur et à mesure que je progressais dans la mise en œuvre,

- j'ai examiné le code et la logique a été mise en œuvre.
...
- J'ai suggéré des améliorations aux prompts système en cours de route, en m'assurant que Claude suivrait bien Git Flow (ce qui n'était pas le cas au début) lors de la création de branches et de commits. Il faut être précis et ne pas supposer que l'AI sait tout.
...
Je me demande si mes crédits se sont épuisés plus rapidement à cause de cela et combien de crédits supplémentaires le mode rapide utilise réellement... Peut-être que la différence de 30 % vient de là ? En attentant, une [recherche Google](https://www.google.com/search?q=claude+code+fast+mode+vs+no+fast+mode) peut peut-être nous aider à trouver la réponse.

À partir de là, je suis passé à l'abonnement Claude Pro.

## Utilisation de Claude Code avec Claude Pro

Même si l'application que je développe est petite, la fenêtre de session de 5 heures peut atteindre 100 % très rapidement (en moins de 2 heures).
...
C'est quelque chose que je n'ai pas encore appliqué, mais dont j'ai pris conscience grâce à mon expérience d'un workflow multi-agents (que je partagerai dans un prochain article).

Un sprint est un backlog de tâches, clairement défini, qu'une équipe doit accomplir dans un certain laps de temps, généralement quelques semaines. Ici, Claude Code dispose d'une fenêtre de 5 heures pour utiliser un certain nombre de jetons et accomplir les tâches. J'ai constaté que, d'une tâche à l'autre, les jetons disponibles s'épuisent plus rapidement ou en plus petite quantité, sans que je puisse déterminer exactement ce qui fait la différence dans un sens ou dans l'autre.
...
J'ai également appris de nouveaux concepts, comme le BFS, e.g. _Breadth-First Search_ en anglais ou la [recherche en largeur](https://en.wikipedia.org/wiki/Breadth-first_search).
